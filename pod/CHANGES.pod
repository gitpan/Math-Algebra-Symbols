...
2004/06/04 1.20 Capitalized module file names correctly.                 

2004/06/04 1.19 Lib, Pod cleanup                                         

2004/06/08 Observed transit of Venus in room converted to Camera Obscura.

2004/06/04 1.18 Equation simplification tied to += operator              

2004/06/02 1.17 Following further excellent suggestions from Mike Schilli
slightly improved notation for equation solving:

B<eq> is a synonym for B<-> 

B<E<gt>> is a synonym for B<solve()>
...
requires/recommends to match MANIFEST.  Removed use of Hash::Util
as this seems to be causing problems with ActiveState.

2004/05/11 1.13 Memory Leak and B<exp()> problems solved:
...
2. Corrected B<exp(-i)*exp(-i)> which was being correctly reduced to
B<exp(0)>, but should have been reduced further to B<1>, which was
causing the multiply out of B<exp(i...)> in B<cot(x) == 1/tan(x)> to fail.
...
improvements.  Added standard Perl Test harness.  Unfixed power()
so we are back to constant powers.
